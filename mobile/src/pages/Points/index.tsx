...
const Points = () => {
  ...
  const [initialPosition, setInitialPotision] = useState<[number, number]>([0, 0,]);
  
  const navigation = useNavigation();


  useEffect(() => {
    async function loadPotision() {
      const { status } = await Location.requestPermissionsAsync();

      if (status !== 'granted') {
        Alert.alert('Opsss... Precisamos de sua permssão para obter a localicação');
        return;
      }
  ...
  function hendleNavigateToDetail() {
